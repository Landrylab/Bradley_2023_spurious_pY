...
# Remove the pSpT kinases from this figure

volc_df <- volc_df[volc_df[,2] != 'Kinase group',]
...
p <- ggplot(volc_df, aes(x=AUC,y=pvalue,color=Group)) + geom_point(size=0.85)
p <- p + scale_color_manual(values = c(rgb(49/255,104/255,142/255,128/255),rgb(53/255,183/255,121/255,128/255), rgb(253/255,231/255,37/255,128/255)))
p <- p + theme_bw() + theme(text=element_text(family="Ubuntu Light"))
p <- p + theme(panel.border = element_blank())
p <- p + geom_segment(aes(x = -6.25, y = 3, xend = -0.175, yend = 3),colour=colors()[308], lwd=0.325, lty=2)
p <- p + geom_segment(aes(x = -0.175, y = 3, xend = -0.175, yend = 6.0),colour=colors()[308], lwd=0.325, lty=2)
p <- p + geom_segment(aes(x = -6.25, y = 6, xend = -0.175, yend = 6),colour=colors()[308], lwd=0.325, lty=2)
p <- p + geom_segment(aes(x = -6.25, y = 3, xend = -6.25, yend = 6.0),colour=colors()[308], lwd=0.325, lty=2)
...
# Add text
p <- p+geom_text(label='p<0.001 \n 22 kinases \n 762 kinase-conditions',x=-6.15,y=3.75, inherit.aes = F, check_overlap = T, size=3.75, hjust=0)

ggsave(file='fitness_volcano.pdf', plot=p, width=4.75, height=3.125)
